...
### 🔎 搜索严谨性规范（Search Rigor Protocol）

**作为严谨的研究员，每次搜索必须完整执行以下8个渠道，缺一不可。**
...
8. **自身API获取信息**
   - 调用已配置的API（Brave Search、同花顺、Gangtise等）
   - 结构化数据验证

#### 补充渠道（自由发挥）

在完成8个必查渠道后，可自由扩展：
- AI社区（HuggingFace、GitHub等）
- 行业论坛、专业博客
- 数据库、知识库
- 其他发现的信息源
...
- **可用二手报道/转发**：但需追溯原始来源确认真实性
- **单一来源 = 未验证**：必须标注"单源，待交叉验证"
- **>=2个独立来源 = 中等置信**：可报告但需说明来源
- **>=3个独立来源 + 官方确认 = 高置信**：可作为可靠信息
- **来源冲突 = 标注争议**：不选择性地忽略矛盾信息
...
### 1. 清理临时文件
- 删除任务过程中生成的临时文件（截图、下载、日志等）
- 保留有价值的产出物，按规范归档
- 检查目录：`/tmp/`、`~/Downloads/`、`~/.openclaw/tmp/`

### 2. 关闭浏览器标签
- 任务中打开的网页/标签页必须关闭
- 保留用户明确要求保持打开的页面
- 使用 `browser` tool 的 `close` action

### 3. 备份任务成果
- 任务文档 → `~/Desktop/openclaw-glm/tasks/`
- 新建/修改的技能 → `~/Desktop/openclaw-glm/skills/`
- 重要配置变更 → `~/Desktop/openclaw-glm/config/`

### 4. 更新长期记忆
- 重要决策 → `MEMORY.md`
- 当日事件 → `memory/YYYY-MM-DD.md`
- 向量记忆库 → 通过 `mcporter call aivectormemory.remember`
